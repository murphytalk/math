...
def translation(x,y):
    ...
    '''
    return Mat(({'x','y','u'},{'x','y','u'}),{('x','x'):1,('x','u'):x,('y','y'):1,('y','u'):y,('u','u'):1})
...
def rotation(angle):
    ...
    '''
    return Mat(({'x','y','u'},{'x','y','u'}),{('x','x'):math.cos(angle),('u','u'):1,('x','y'):-math.sin(angle),('y','x'):math.sin(angle),('y','y'):math.cos(angle)})
...
def grayscale():
    ...
    '''
    label={'r','g','b'}
    return Mat((label,label),{ (x,y):77/256 if (y=='r') else ( 151/256 if (y=='g') else 28/256 ) for x in label  for y in label })
    #(77/256,151/256,28/256)
