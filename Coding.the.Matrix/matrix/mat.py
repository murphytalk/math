...
def add(A, B):
    ...
    assert A.D == B.D
    d = { k:A.f[k] + (B.f[k] if k in B.f.keys() else 0)  for k in A.f.keys() }
    for k in B.f.keys():
        if k not in A.f:
            d[k] = B.f[k]
    return Mat(A.D,d)
...
def equal(A, B):
    ...
    #for any key only in A or B, the value has to be 0
    for k in ka - kb:
        if A.f[k]!=0:
            return False
...
def transpose(M):
    ...
    return Mat((M.D[1],M.D[0]),{ (j,i):M.f[(i,j)] for i,j in M.f.keys()})
