...
## Problem 1
# can be solved by using hw4.exchange
w0 = list2vec([1,0,0])
w1 = list2vec([0,1,0])
w2 = list2vec([0,0,1])
...
def morph(S, B):
    ...
    ret = []
    A = []
    SS = [x for x in S] #the grader does not like S being touched
    for z in B:
        w = exchange(SS,A,z)
        if w is None:
            break
...
row_space_1 = [ list2vec(x) for x in [[1,2,0],[0,2,1]] ]
col_space_1 = [ list2vec(x) for x in [[1,0],[0,1]] ]

row_space_2 = [ list2vec(x) for x in [[1,4,0,0],[0,2,2,0],[0,0,1,1]] ]
col_space_2 = [ list2vec(x) for x in [[1,0,0],[0,2,1],[0,0,1]] ]

row_space_3 = [ list2vec(x) for x in [[1]]]
col_space_3 = [ list2vec(x) for x in [[1,2,3]] ]

row_space_4 = [ list2vec(x) for x in [[1,0],[2,1]] ]
col_space_4 = [ list2vec(x) for x in [[1,2,3],[0,1,4]] ]
