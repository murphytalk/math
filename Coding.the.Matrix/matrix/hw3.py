...
def dot_product_mat_vec_mult(M, v):
    assert(M.D[1] == v.D)
    rows = mat2rowdict(M)
    return Vec(M.D[0],{k: vec*v for k,vec in rows.items()})
...
def dot_product_vec_mat_mult(v, M):
    assert(v.D == M.D[0])
    cols = mat2coldict(M)
    return Vec(M.D[1],{k: vec*v for k,vec in cols.items()})
...
def Mv_mat_mat_mult(A, B):
    ...
    d = {k:A*vec for k,vec in b_cols.items()}
...
def dot_prod_mat_mat_mult(A, B):
    assert A.D[1] == B.D[0]
    return Mat((A.D[0],B.D[1]),{ (i,j): Vec(A.D[1],{ jj:A[ii,jj] for ii,jj in A.f.keys() if ii==i}) * Vec(B.D[0],{ii:B[ii,jj] for ii,jj in B.f.keys() if jj==j} ) for i in A.D[0] for j in B.D[1]})
...
solving_systems_a_times_m = Mat(({0, 1}, {0, 1}), { (1, 0): 0.0, (0, 0): 1.0, (1, 1): 1.0})
solving_systems_m_times_a = Mat(({0, 1}, {0, 1}), { (0, 1): 0.0, (0, 0): 1.0, (1, 1): 1.0})
